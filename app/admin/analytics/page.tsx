...
export default async function AdminAnalyticsPage({
  ...
  const days: AnalyticsPeriod =
    params.days === '7' || params.days === '90' ? Number(params.days) as AnalyticsPeriod : 30;
      ...
      <main className="min-h-screen bg-slate-950 px-4 py-6 text-slate-100 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <section className="mt-7 overflow-hidden rounded-2xl border border-blue-800/70 bg-gradient-to-br from-blue-950/80 via-slate-900 to-slate-900 p-6">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-blue-300">AI INSIGHT</p>
              ...
              {data.dailyData.length === 0 ? (
                ...
                <div className="mt-8 flex h-64 items-end gap-2 overflow-x-auto pb-2">
                  {data.dailyData.map((item) => (
                    <div key={item.date} className="flex min-w-8 flex-1 flex-col items-center justify-end">
                      <span className="mb-2 text-xs font-black text-slate-300">{item.views}</span>
                    ...
                    <div key={source.name}>
                      <div className="mb-2 flex items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">세션 {formatNumber(source.sessions)}회</p>
                        </div>
                        <p className="text-sm font-black text-slate-200">{source.percentage.toFixed(1)}%</p>
                      </div>
                      <div className="h-2.5 overflow-hidden rounded-full bg-slate-800">
                        <div className="h-full rounded-full bg-blue-500" style={{ width: `${source.percentage}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <table className="w-full min-w-[760px] border-collapse">
                  <thead>
                    <tr className="border-b border-slate-800 text-left">
                      ...
                      <th className="px-3 py-3 text-xs text-slate-500">콘텐츠</th>
                      <th className="px-3 py-3 text-right text-xs text-slate-500">조회수</th>
                      <th className="px-3 py-3 text-right text-xs text-slate-500">사용자</th>
                      <th className="px-3 py-3 text-right text-xs text-slate-500">평균 참여</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.popularPosts.map((post, index) => (
                      <tr key={`${post.path}-${index}`} className="border-b border-slate-800/70 last:border-0">
                        <td className="px-3 py-4 font-black text-blue-300">{index + 1}</td>
                        <td className="px-3 py-4">
                          <p className="max-w-xl truncate font-black text-white">{post.title}</p>
                          <p className="mt-1 max-w-xl truncate text-xs text-slate-500">{post.path}</p>
                        </td>
                        <td className="px-3 py-4 text-right font-black">{formatNumber(post.views)}</td>
                        <td className="px-3 py-4 text-right text-slate-300">{formatNumber(post.users)}</td>
                        <td className="px-3 py-4 text-right text-slate-300">{formatDuration(post.averageTime)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <section className="mt-7 grid grid-cols-1 gap-6 lg:grid-cols-2">
            <article className="rounded-2xl border border-slate-800 bg-slate-900 p-5 sm:p-6">
              ...
              <div className="mt-6 grid gap-4 sm:grid-cols-2">
                <EventCard label="📖 글 스크롤" value={data.events.articleScroll} />
                <EventCard label="🔗 내부 링크 클릭" value={data.events.internalLinkClick} />
              </div>
            </article>
            ...
            <article className="rounded-2xl border border-slate-800 bg-slate-900 p-5 sm:p-6">
              <SectionTitle
                ...
                title="유입 검색어"
                description="검색어는 GA4만으로 정확히 제공되지 않습니다."
              />
              <div className="mt-6 rounded-xl border border-amber-800/60 bg-amber-950/30 p-5 text-sm leading-6 text-amber-100/80">
                가짜 예시값은 제거했습니다. 다음 단계에서 Google Search Console과 네이버 Search Advisor를 연결하면 실제 검색어·노출·클릭·순위를 표시할 수 있습니다.
              </div>
            </article>
          </section>

          <section className="mt-7 rounded-2xl border border-slate-800 bg-slate-900 p-5 sm:p-6">
            <SectionTitle eyebrow="DATA CONNECTION" title="데이터 연결 상태" description="외부 분석 서비스 연동 현황입니다." />
            <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
              <Connection name="Google Analytics" description="방문자·체류·행동" connected />
              <Connection name="Search Console" description="검색어·노출·순위" />
    ...
    return (
      <main className="min-h-screen bg-slate-950 px-4 py-10 text-slate-100">
        <div className="mx-auto max-w-2xl rounded-2xl border border-red-900/70 bg-red-950/30 p-7">
          ...
          <h1 className="mt-2 text-2xl font-bold">방문 분석 데이터를 불러오지 못했습니다</h1>
          <p className="mt-4 rounded-xl bg-slate-950 p-4 text-sm text-slate-300">{message}</p>
          <Link href="/admin" className="mt-6 inline-block rounded-xl border border-slate-700 px-4 py-2 text-sm">
            ← 관리자 센터
          </Link>
        </div>
      </main>
...
function EventCard({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-4">
      ...
      <strong className="mt-3 block text-2xl text-emerald-400">{formatNumber(value)}</strong>
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className={`rounded-full px-2.5 py-1 text-[11px] font-black ${connected ? 'bg-emerald-950 text-emerald-300' : 'bg-slate-800 text-slate-400'}`}>
          {connected ? '연결됨' : '연결 전'}
        </span>
      </div>
      <p className="mt-2 text-xs text-slate-500">{description}</p>
