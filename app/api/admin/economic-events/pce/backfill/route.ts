...
import { runPceHistoricalBackfill } from "@/app/lib/pceEventDb";
...
export async function POST(request: NextRequest) {
  if (!(await verifyAdminRequest(request))) {
    return NextResponse.json({ ok: false, error: "관리자 로그인이 필요합니다." }, { status: 401, headers: { "Cache-Control": "no-store" } });
  }

  try {
    const result = await runPceHistoricalBackfill();
