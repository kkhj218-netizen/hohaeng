...
import MajorFuturesPanel from "@/app/today/MajorFuturesPanel";
...
function formatValue(quote: UsMarketCloseQuote) {
  ...
  return new Intl.NumberFormat("ko-KR", { maximumFractionDigits: digits }).format(quote.current);
}
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="text-xs font-bold uppercase tracking-wide text-slate-400">{quote.symbol}</p>
          <p className="mt-1 truncate text-sm font-black text-slate-900">{quote.name}</p>
        </div>
        <p className={`shrink-0 text-sm font-black tabular-nums ${tone(quote.changePercent)}`}>{formatPercent(quote.changePercent)}</p>
      </div>
      <p className="mt-4 text-xl font-black tracking-tight tabular-nums text-slate-950">{formatValue(quote)}</p>
      <p className="mt-1 text-[11px] leading-4 text-slate-400">{formatDate(quote.date)} · 미국 정규장 마감</p>
      {(quote.sessionHigh !== null || quote.sessionLow !== null) && (
        <div className="mt-3 border-t border-slate-100 pt-2 text-[10px] text-slate-500">
          ...
          {quote.rangePercent !== null && <p className="mt-1 font-bold text-slate-600">장중 범위 {quote.rangePercent.toFixed(2)}%</p>}
        </div>
      )}
    </div>
  );
...
function directionMeta(cash: number | null, future: number | null) {
  if (cash === null || future === null) return { label: "비교 대기", tone: "bg-slate-100 text-slate-600", delta: null };
  const delta = future - cash;
  const cashSign = cash > 0.03 ? 1 : cash < -0.03 ? -1 : 0;
  const futureSign = future > 0.03 ? 1 : future < -0.03 ? -1 : 0;
  if (cashSign === 0 && futureSign === 0) return { label: "중립 동행 →", tone: "bg-slate-100 text-slate-700", delta };
  if (cashSign !== futureSign) return futureSign > cashSign
    ? { label: "선물 상대강세 ↗", tone: "bg-violet-50 text-violet-700", delta }
    : { label: "선물 상대약세 ↘", tone: "bg-amber-50 text-amber-700", delta };
...
export default async function UsMarketClosePanel({
  ...
  const resolvedMarket = market ?? { generatedAt: "", cash: [], futures: [] };
  const resolvedFutures = majorFutures ?? [];

  if (resolvedMarket.cash.length === 0 && resolvedFutures.length === 0) {
    return <p className="mt-5 rounded-2xl bg-slate-50 p-4 text-sm text-slate-500">저장된 최근 미국시장 마감 데이터를 확인 중입니다.</p>;
  }

  const latestDate = [...resolvedMarket.cash, ...resolvedFutures].map((item) => item.date).sort((a, b) => b.localeCompare(a))[0];
  const cashMap = new Map(resolvedMarket.cash.map((item) => [item.symbol, item]));
  const futureMap = new Map(resolvedFutures.filter((item) => item.group === "index").map((item) => [item.symbol, item]));
  const paired = PAIRS.map((pair) => ({ ...pair, cashQuote: cashMap.get(pair.cash), futureQuote: futureMap.get(pair.future) })).filter((pair) => pair.cashQuote && pair.futureQuote);

  return (
    <>
      <div className="mt-5 flex flex-wrap items-center justify-between gap-2">
        <p className="text-xs font-bold text-slate-500">현물 정규장 종가</p>
        {latestDate && <span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-black text-emerald-700">최근 마감 {formatDate(latestDate)}</span>}
      </div>

      {resolvedMarket.cash.length > 0 && (
        <div className="mt-3 grid grid-cols-2 gap-3 md:grid-cols-4">
          {resolvedMarket.cash.map((quote) => <CashCard key={quote.symbol} quote={quote} />)}
        </div>
      )}

      <MajorFuturesPanel futures={resolvedFutures} />

      {paired.length > 0 && (
        <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-950 p-4 text-white">
          <div className="flex flex-wrap items-end justify-between gap-2">
            <div>
              <p className="text-xs font-black uppercase tracking-wider text-cyan-300">DIRECTION MAP</p>
                ...
                <div key={pair.future} className="rounded-xl border border-white/10 bg-white/[0.05] p-3">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="mt-1 text-[10px] text-slate-400">현물 {formatPercent(cashChange)} · 선물 {formatPercent(futureChange)}</p>
                    </div>
                    <span className={`rounded-full px-2.5 py-1 text-[10px] font-black ${meta.tone}`}>{meta.label}</span>
                  </div>
                  <p className="mt-2 text-xs font-bold text-slate-300">상대 변화차 {formatPoint(meta.delta)}</p>
                </div>
              );
            })}
          </div>
          <p className="mt-3 text-[10px] leading-4 text-slate-500">상대 변화차는 현물과 선물의 등락률 차이일 뿐 공정가치 베이시스가 아닙니다. 특히 NQ는 나스닥100 선물이고 현물 카드는 나스닥 종합지수라 방향·상대강도 참고용으로만 봅니다.</p>
        </div>
      )}

      <MarketMapPreview earningsRisk={earningsRisk} />

      <Link href="/data/regime" className="mt-5 block rounded-2xl border border-blue-200 bg-gradient-to-r from-blue-50 to-white p-4 transition hover:border-blue-300 hover:shadow-sm">
      ...
      <FearGreedPanel reading={fearGreed} />
